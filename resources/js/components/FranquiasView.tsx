...
export function FranquiasView({ checklists, onSelect, onCreate }: FranquiasViewProps) {
    const [activeTab, setActiveTab] = useState<'ANDAMENTO' | 'CONCLUIDO'>('ANDAMENTO');
    const [searchTerm, setSearchTerm] = useState('');

    // Filter for Franquias department
    const departmentChecklists = checklists.filter(c => {
        const data = c.data || {};
        return data.setor_departamento === 'Franquias' || c.type.includes('Franquias');
    });
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-serif text-rose-gold-dark flex items-center gap-2">
                        <Store className="w-8 h-8 text-rose-gold" />
                        Franquias - Gestão
                    </h1>
                    <p className="text-muted-foreground mt-1">
                        Acompanhe unidades e processos de franquias
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        onClick={() => {
                            const title = prompt("Título do Processo:", "Nova Unidade/Processo");
                            if (title) onCreate('franquias', title);
                        }}
